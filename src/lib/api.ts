...
export const api = {
  ...
  setUserLimit: (username: string, access_duration: string, month_count: number | undefined, day_count: number | undefined, year_count: number | undefined, currentUser: { username: string; role: string; office_name: string }) =>
    patch('/api/users', { action: 'setLimit', username, access_duration, month_count, day_count, year_count, currentUsername: currentUser.username, currentRole: currentUser.role, currentOffice: currentUser.office_name }),
