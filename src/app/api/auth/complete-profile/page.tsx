...
function MathCaptcha({ onVerify }: { onVerify: (ok: boolean) => void }) {
  ...
  const verify = () => { const correct = parseInt(ans) === a + b; setOk(correct); setChecked(true); onVerify(correct) }
  return (
    <div className="flex items-center gap-2 bg-slate-50 border border-slate-100 rounded-xl px-3 py-2">
      <span className="text-xs font-bold text-slate-500">ยืนยันตัวตน: {a} + {b} = ?</span>
      <input value={ans} onChange={e => { setAns(e.target.value.replace(/\D/g, '')); setChecked(false); onVerify(false) }}
        className="w-12 border border-slate-200 rounded-lg px-2 py-1 text-xs text-center font-bold" maxLength={2} placeholder="?" />
      <button type="button" onClick={verify} className="px-2 py-1 bg-slate-200 text-slate-600 rounded-lg text-xs font-bold hover:bg-slate-300">ตรวจ</button>
      {checked && <span className={`text-xs font-bold ${ok ? 'text-green-600' : 'text-red-500'}`}>{ok ? '✓' : '✗'}</span>}
    </div>
  )
}
...
function CompleteProfileForm() {
  ...
  const handleSubmit = async () => {
    ...
    if (!captchaOk) { setError('กรุณายืนยัน Captcha ก่อน'); return }
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md p-8 animate-fadeIn" style={{ maxHeight: '90vh', overflowY: 'auto' }}>
        ...
        <button onClick={handleSubmit} disabled={loading || !captchaOk}
