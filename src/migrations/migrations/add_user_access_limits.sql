...
ALTER TABLE users
  ...
  ADD COLUMN IF NOT EXISTS access_month_count INTEGER,  -- only used when access_duration = 'month'
  ADD COLUMN IF NOT EXISTS access_start       TIMESTAMPTZ,
  ADD COLUMN IF NOT EXISTS access_until       TIMESTAMPTZ;

-- access_duration    : the duration type set by global_admin
-- access_month_count : number of months (e.g. 3, 6, 12) when duration = 'month'
